...
from generators.utils.transformers import DataTransformer
...
class CTGAN(BaseSynthesizer):
    ...
    def fit(
        ...
        self._transformer = DataTransformer(numerical_preprocess=numerical_preprocess)
...
class CTGANDataGenerationPipeline(BaseDataGenerator):
    def __init__(self, config: Dict[str, Any], split_no: int = 1):
        ...
        self.experiment_name = (
            f"pp_{self.preprocess}-bs_{self.batch_size}-iters_{self.num_iters}"
        )
    ...
    def train(self):
        ...
        self.model.fit(
            train_data, discrete_columns, numerical_preprocess=self.preprocess
        )
        self.model.save(
...
def run_pipeline(config_path: str, split_no: int):
    ...
    if config["generator_name"] != "ctgan":
        config["generator_name"] = "ctgan"

    pipeline = CTGANDataGenerationPipeline(config, split_no=split_no)
